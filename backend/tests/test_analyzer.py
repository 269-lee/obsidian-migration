...
@pytest.mark.asyncio
async def test_analyze_returns_schema():
    with patch("ai.analyzer.anthropic.AsyncAnthropic") as mock_anthropic:
        ...
        result = await analyzer.analyze(documents, ["Projects", "Areas", "Resources", "Inbox"])
...
@pytest.mark.asyncio
async def test_analyze_calls_claude_with_documents():
    with patch("ai.analyzer.anthropic.AsyncAnthropic") as mock_anthropic:
        mock_client = MagicMock()
        mock_anthropic.return_value = mock_client
        mock_client.messages.create = AsyncMock(return_value=MagicMock(
        ...
        await analyzer.analyze(documents, ["Projects"])
